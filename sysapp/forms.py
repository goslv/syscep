...
class UsuarioForm(forms.ModelForm):
    ...
    sede = forms.ModelChoiceField(
        ...
        help_text='Solo aplica a usuarios no administradores. '
                  'Dejar vacío para acceso a todas las sedes.',
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.instance.pk:
            ...
            # Prellenar sede desde el perfil existente
            try:
...
class PagoForm(forms.ModelForm):
    ...
    # ── Carrera libre ("Otros") ───────────────────────────────────────────
    # Cuando el usuario selecciona "Otros" en el select de carrera,
    # puede escribir un nombre libre aquí.
    carrera_otro = forms.CharField(
        required=False,
        max_length=200,
        label='Especificar carrera o curso',
        widget=forms.TextInput(attrs={
            'class': 'form-control',
            'id':    'id_carrera_otro',
            'placeholder': 'Ej: Inglés avanzado, Peluquería…',
        })
    )
    ...
    class Meta:
        ...
        widgets = {
            ...
            'numero_cuota': forms.TextInput(attrs={
                'class': 'form-control', 'placeholder': 'Opcional. Ej: 3,4,5',
            }),
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Carrera: queryset normal + opción "OTROS" al final
        carrera_qs = Carrera.objects.filter(activa=True).order_by('nombre')
        ...
        self.fields['carrera_otro'].required    = False
        ...
        if self.instance.pk:
            ...
            # carrera_otro: si el pago tiene un valor libre en carrera_texto (si existe el campo)
            if hasattr(self.instance, 'carrera_otro') and self.instance.carrera_otro:
                self.initial['carrera_otro'] = self.instance.carrera_otro
    ...
    def clean(self):
        ...
        carrera_otro            = cleaned_data.get('carrera_otro', '').strip()
...
class CarreraForm(forms.ModelForm):
    class Meta:
        model = Carrera
        fields = [
            'nombre', 'naturalidad', 'duracion_meses', 'monto_matricula',
...
class MateriaForm(forms.ModelForm):
    class Meta:
        model = Materia
        fields = [
            'nombre', 'bimestre', 'orden', 'fecha_examen_parcial',
            'fecha_examen_final', 'link_classroom', 'docente',
        ]
        widgets = {
            'nombre':   forms.TextInput(attrs={'class': 'form-control'}),
            ...
            'link_classroom': forms.URLInput(attrs={
                'class': 'form-control', 'placeholder': 'https://classroom.google.com/...',
            }),
